...
function ConversationPanel({ proposal }: { proposal: ApiProposalList }) {
  ...
  const { provider, client } = convData;
  const isCurrentUserProvider = user?.id === provider.id;
  const otherUser = isCurrentUserProvider ? client : provider;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-3 border-b border-border shrink-0 bg-white">
        <div className="flex items-center gap-3">
          {!isCurrentUserProvider && provider.provider_profile_id ? (
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-8 text-center gap-2">
                  ...
                  <p className="text-xs opacity-60">
                    Les conversations apparaissent après examen de vos propositions.
                  </p>
                </div>
              ) : (
                      ...
                      >
                        <div className="flex items-start gap-2.5">
                          <div className="w-9 h-9 rounded-full bg-primary/10 text-primary font-bold text-sm flex items-center justify-center flex-shrink-0 mt-0.5">
                            {proposal.project.title.charAt(0).toUpperCase()}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-1 mb-0.5">
                              <p className="font-medium text-sm leading-snug line-clamp-1 flex-1">
                                {proposal.project.title}
                              </p>
                              <Badge
                                variant="outline"
              ...
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-3">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs mt-0.5 opacity-60">
                    Choisissez une proposition dans la liste
                  </p>
                </div>
              </div>
            )}
